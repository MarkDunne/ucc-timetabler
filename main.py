...
#Programmes are the courses that run over the whole academic year
class Programme(db.Model):
    code = db.StringProperty(required=True)

#Modules are individual classes than run for one semester
...
class AuthHandler(webapp2.RequestHandler):
    def get(self):
        ...
        if code:
            ...
            #from here everything is done over AJAX 
            self.redirect('/landing?' + request)
        #should do more error checking here

#main part of the application
#check to see if module is already in database
#if it is, use it
#else, download and parse a copy of the timetable then push data to database
class ModuleHandler(webapp2.RequestHandler):
    ...
    #run through the timetable html and extract all data on the modules
    #then push them all to a database
    def parse(self):
        ...
        modules = []
        for row in rows:
            #variable to keep track of which time block we're in
            timePosition = 0
    ...
    #if the programme has already been parsed and on the database, return that. else start a new parsing
    def get(self):
...
#calender operations
#creates a new calender and puts all of the select modules inside
...
class FinalizeHandler(webapp2.RequestHandler):
    ...
    #check if calender already exists
    ...
    #for debugging, dont actually delete any calender
    ...
    def putModule(self, module):
        ...
        #send to calender asynchronously and in parallel
        #no clear api documentation for sending batch events but that would be better
    ...
    def get(self):
        ...
        if self.access_token and self.programme_code and self.choices:
            try:
                ...
                #wait for all modules to be sent up
...
#provides convenient way to clear db
